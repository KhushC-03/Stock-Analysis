...
c = CurrencyConverter()
apiusrnme = bytes.fromhex('').decode('utf-8')
apikey = bytes.fromhex('').decode('utf-8')
os.system('cls')
v = requests.session()
def get_stock_price():
...
def share_price_notifications():
    ...
    if settings == False:
        ...
        email = data[2]['clicksend_username']
        api_key = data[2]['clicksend_apikey']
        I1 = input("What are the stock(s) you'd like to monitor? If there are multiple stocks split them with a comma ")
        I2a = input('What are the stock price you want to be notified at? ')
        if ',' in I1:            
            stock_comma = I1.split(',')
            for i, stocks in enumerate(stock_comma, start=1):  
                I2 = I2a.split(',')[i - 1]             
                note = ('{} stock has hit ${}'.format(stocks,I2))
                t = datetime.utcnow()
                ...
                log.write(log0)
                thread = threading.Thread(target=thread_monitor,args=(i,I1,stocks,delay,I2,email,api_key,note,number,))
                thread.start()
            ...
            while True:
                ...
                stock_price = v.get(base_url, timeout=5, allow_redirects=False, verify=True,proxies=proxies,)
                soup = BeautifulSoup(stock_price.content, 'html.parser')
                webdata = soup.find_all('span')[20]
                ...
                if 201 <= stock_price.status_code <= 1000:                                    
                    ...
                    if webdata_price >= I2:
                        ...
                        configuration.username = str(email)
                        configuration.password = str(api_key)
...
def thread_monitor(i,I1,stocks,delay,I2,email,api_key,note,number,):
    ...
    while True:       
        ...
        webdata = soup.find_all('span')[20]
        webdata_price = webdata.text
        print('\nMonitor loop at '+str(i)+' try(s)','Current share price at $'+str(webdata_price),end='\r',)
        time.sleep(int(delay))
        if 201 <= stock_price.status_code <= 1000:                                    
            timestamp2 = time.strftime('%H:%M:%S')
            log1 = ('[{}] [MONITOR LOOP {}] [ERROR] [{}] [RETRY MONITOR FAILED RESTART TASK] [STOCK PRICE AT {}]\n').format(timestamp2,i,stock_price.status_code,webdata_price)
